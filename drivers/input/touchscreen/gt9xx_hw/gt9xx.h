...
#define GTP_FW_FORCE_UPDATE_FLAG     0xBE

#if GTP_SLIDE_WAKEUP
#define DOUBLE_CLICK_WAKEUP  0xCC
#define LINEAR_SLIDE_LEFT_TO_RIGHT 0xAA
#define LINEAR_SLIDE_RIGHT_TO_LEFT 0xBB
#define LINEAR_SLIDE_TOP_TO_BOTTOM 0xAB
#define LINEAR_SLIDE_BOTTOM_TO_TOP 0xBA
...
#define IS_GTP_GESTURE_ENABLE(gesture, var) (IS_APP_ENABLE_GESTURE((gesture)) & (var))
#define REPORT_GESTURE_LOCUS 1
#define GTP_LOCUS_DATA_LENS 4
#define GTP_LETTER_LOCUS_NUM 8
#define GTP_LINEAR_LOCUS_NUM 2
#define GTP_GESTURE_POINT_REG 0x814C
#define GTP_LOCUS_DATA_NUM 33
...
do{                                     \
    if( goodix_debug_mask >= TP_WARNING )  \
    {                                   \
         pr_err("[GTP_WARNING] " x);  \
    }                                   \
                                        \
}while(0)
#endif
...
enum gtp_module_id {
	OFILM_ID = 0,
	NEW_OFILM_ID = 1,
	UNKNOW_SENSOR_ID = 0xff,
};

struct gtp_wakeup_keys {
	const uint16_t  *keys;
	uint8_t         size;
...
struct goodix_ts_platform_data {
	...
	u32 panel_maxx;
	u32 panel_maxy;
	struct gtp_wakeup_keys *wakeup_keys;
...
/* the config for new pattern, sensor ID is 1*/
#define CTP_CFG_GROUP2 {\
	0x4A,0xD0,0x02,0x00,0x05,0x05,0x35,0x01,0x02,0xC8,\
	0x19,0x0A,0x55,0x3C,0x03,0x50,0x00,0x00,0x22,0x53,\
	0x12,0x11,0x05,0x15,0x19,0x23,0x14,0x85,0x25,0xBB,\
	0x81,0x83,0x7C,0x06,0x00,0x00,0x00,0x02,0x33,0x11,\
	0x00,0x55,0x03,0x04,0x17,0x02,0x2B,0x04,0x00,0x00,\
	0x00,0x7A,0x8E,0x4A,0xE5,0x04,0x07,0x00,0x00,0x04,\
	0x85,0x7B,0x00,0x81,0x7F,0x00,0x7E,0x83,0x00,0x7A,\
	0x87,0x00,0x77,0x8B,0x00,0x77,0x18,0x38,0x50,0x00,\
	...
	0x00,0x41,0x01,0x00,0x0A,0x96,0x2B,0x01\
}
