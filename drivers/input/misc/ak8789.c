...
typedef enum hall_used_type{
	/*for only one hall devices: connect only one pin another pin floating, such as G760-UL00*/
	ONE_POLE_ONE_PIN = 0,
	/*for only one hall devices: two pins connect, such as G660S*/
	ONE_POLE = 1,
	/*for two hall devices: connect only one pin another pin floating for each hall devices, such as G760-TL00*/
	TWO_POLE = 2,
	/*for two hall devices: two pins connect for each hall devices, such as Mate2*/
	FOUR_POLE = 4,
} hall_used_type_t;
...
static int hall_parse_dt(struct device *dev)
{
	...
	/*for G760_UL*/
	 gpio_data_t  one_pole_config_for_g760_ul[1]={
		[0] ={
			.wake_up 		= IRQF_NO_SUSPEND,
			.name		= GPIO_CONFIG_COVER,
			.hall_value 	= HALL_VALUE_RIGHT_NORTH,
	...
	switch(used_type)
	{
		case ONE_POLE_ONE_PIN:
			which_pole = one_pole_config_for_g760_ul;
