...
static int32_t msm_ois_turn_onoff(struct msm_ois_ctrl_t *o_ctrl, uint32_t onoff)
{
	...
	/*mini-isp do the ois init*/
    if(o_ctrl->ois_onoff == onoff)
    {
	pr_err("%s: have done return \n", __func__);
	return rc;
    }
...
static int32_t msm_ois_mmi_test(struct msm_ois_ctrl_t *o_ctrl)
{
    int32_t ret = 0;
    ...
    ret = mini_isp_ois_start_mmi_test(o_ctrl->module_id);

    o_ctrl->check_ois = ret;
    o_ctrl->ois_wait_end = 1;
    wake_up_interruptible(&o_ctrl->wait_check_ois);
...
#endif
        {
            pr_err("CFG_OIS_START_MMI_CHECK ois don't init complete! \n");
            break;
        }
        rc = msm_ois_mmi_test(o_ctrl);
        if (rc < 0)
			pr_err("CFG_OIS_START_MMI_CHECK failed %d\n", rc);
        break;
