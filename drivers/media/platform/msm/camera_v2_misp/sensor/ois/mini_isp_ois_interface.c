...
int mini_isp_ois_getotp(uint8_t sensor, uint8_t *otpInfo, uint32_t len);

#define ISO_IMX278_OTP_DATA_LENGTH 8
#define LSC_IMX278_OTP_DATA_LENGTH 1201
#define VCM_IMX278_OTP_DATA_LENGTH 4

#define OIS_IMX278_OTP_DATA_LENGTH 33
#define OIS_IMX278_OTP_DATA_LENGTH_LGIT 53
...
typedef struct {
    ...
    uint8_t   aucISO_AWBCalib[ISO_IMX278_OTP_DATA_LENGTH];
    uint8_t   aucLSC[LSC_IMX278_OTP_DATA_LENGTH];
    uint8_t   aucVCM[VCM_IMX278_OTP_DATA_LENGTH];
    uint8_t   aucOIS[OIS_IMX278_OTP_DATA_LENGTH];
    uint8_t   ucDataValid;
}OTP_IMX278_MODULE_DATA_SUNNY_LITEON;
...
typedef struct {
    ...
    uint8_t   aucLSC[LSC_IMX278_OTP_DATA_LENGTH];
    uint8_t   aucVCM[VCM_IMX278_OTP_DATA_LENGTH];
    uint8_t   aucOIS[OIS_IMX278_OTP_DATA_LENGTH_LGIT];
    uint8_t   ucDataValid;
}OTP_IMX278_MODULE_DATA_LGIT;
...
int32_t mini_isp_ois_init_otp(int32_t module_id)
{
 #if 1
    uint8_t *ois_otp = NULL;
    int rc = 0;
    struct msm_imx278_ois_fn_t *ois_func = NULL;
    uint32_t ois_otp_length = OIS_IMX278_OTP_DATA_LENGTH;
    uint8_t *p_ois_otp_data = NULL;
    int i = 0;
    int32_t total_otp_len = 0;

    switch(module_id)
    {
        case SUNNY_MODULE_ID:
            ois_otp_length  = OIS_IMX278_OTP_DATA_LENGTH;
            ois_func = &imx278_sunny_ois_func;
            total_otp_len = sizeof(OTP_IMX278_MODULE_DATA_SUNNY_LITEON);
            ois_otp = (uint8_t *)kmalloc(total_otp_len,GFP_KERNEL);
            break;
        case LITEON_MODULE_ID:
            ois_otp_length  = OIS_IMX278_OTP_DATA_LENGTH;
            ois_func = &imx278_liteon_ois_func;
            ...
            break;
        case LGIT_MODULE_ID:
            ois_otp_length  = OIS_IMX278_OTP_DATA_LENGTH_LGIT;
            ois_func = &imx278_lgit_ois_func;
            total_otp_len = sizeof(OTP_IMX278_MODULE_DATA_LGIT);
            ois_otp = (uint8_t *)kmalloc(total_otp_len,GFP_KERNEL);
            break;
    ...
    pr_info("%s: start get otp info \n",__func__);
    rc = mini_isp_ois_getotp(0,ois_otp,total_otp_len);
    if(rc < 0)
    {
        kfree(ois_otp);
        ois_otp = NULL;
        pr_err("%s: mini_isp_ois_get_otp fail \n",__func__);
        return rc;
    }

    if(module_id == LGIT_MODULE_ID)
    {
        OTP_IMX278_MODULE_DATA_LGIT *otp_data_lgit = NULL;
        otp_data_lgit = (OTP_IMX278_MODULE_DATA_LGIT *)ois_otp;
        p_ois_otp_data = otp_data_lgit->aucOIS;
        pr_info("%s: otp valid = %d vendor=%d \n",__func__,(int)otp_data_lgit->ucDataValid,(int)otp_data_lgit->ucVenderAndVersion);
    }
    else
    {
        OTP_IMX278_MODULE_DATA_SUNNY_LITEON *otp_data = NULL;
        otp_data = (OTP_IMX278_MODULE_DATA_SUNNY_LITEON *)ois_otp;
        p_ois_otp_data = otp_data->aucOIS;
        pr_info("%s: otp valid = %d vendor=%d \n",__func__,(int)otp_data->ucDataValid,(int)otp_data->ucVenderAndVersion);
    }

    for(i = 0; i< ois_otp_length; i++)
    {
        pr_info("ois_otp[%d] = 0x%x \n",i,p_ois_otp_data[i]);
    }

    
    if(ois_func && ois_func->init_ois_otp)
    {
        ois_func->init_ois_otp((void *)p_ois_otp_data, ois_otp_length);
    }
...
int32_t mini_isp_ois_start_mmi_test(int32_t module_id)
{
   int32_t ret = 0;
   uint8_t retgea = 0, rethea = 0;
   struct msm_imx278_ois_fn_t *ois_func = check_module_id(module_id);
   ...
   if(!ois_func)
   {
    ...
    mini_isp_ois_WitTim(100);
    ois_func->SetPanTiltMode(0);
    mini_isp_ois_WitTim(10);
    /* Move Lens outside from AF dead area */
    ois_func->AF_SetPos(300);

    /* close OIS */
    ois_func->RtnCen(0x00);			// Lens centering
	mini_isp_ois_WitTim(100);		// Wait Stable
	/* start test*/
    ois_func->RunGea();
    ois_func->RunHea();
    pr_info("%s:retgea = %d  rethea = %d \n", __func__,retgea,rethea);
    if((retgea == 0x02) && (rethea == 0x02)) {
        ret = 0x02;
    } else {
        ret = ((rethea&0xfc) << 4) | (retgea&0xfc);
    }
    pr_info("%s:ret = %d \n", __func__,ret);

    /* set ois running*/
    //move lens outside from AF dead area
    ois_func->AF_SetPos(100);
    //open OIS
    //lens centering
    ois_func->RtnCen(0x00);
...
{
    int errorcode = 0;
    uint32_t OpCode = ISPCMD_BULK_GET_OTP_DATA;    
    uint32_t cmdLen = 9, block = 8192;
    uint8_t in_buf[9] = {0};

    in_buf[0] = sensor; //0 for back camera
    memcpy(&(in_buf[1]), &len ,sizeof(len));    
    memcpy(&(in_buf[5]), &block, sizeof(block));
    
    errorcode = misp_exec_bidir_cmd(OpCode,in_buf,cmdLen,1,otpInfo,len);
    if(errorcode)
    {
        pr_err("%s: fail\n",__func__);
    }
