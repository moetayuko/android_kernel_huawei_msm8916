...
static bool ov13850_otp_read_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_debug("module info year 20%02d month %d day %d, SNO. 0x%x  vendor id&version 0x%x\n", buf[0],buf[1],buf[2],buf[3],buf[4]);
...
static bool ov13850_otp_read_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	//pr_debug("%s OTP data are Rg_high=%x, Rg_low=%x, Bg_high=%x, Bg_low=%x, gbgr_high=%x, gbgr_low=%x!!!\n", __func__, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5]);
...
static void ov13850_otp_set_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_debug("ov13850_get_otp_from_sensor() ~ %s R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n",__func__, R_gain, B_gain, G_gain);
       /* awb set flag */
...
static bool ov13850_otp_read_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_debug("%s LCS[0]=%x, LSC[244]=%x, LSC[245]=%x, LSC[359]=%x\n",__func__,
...
static void ov13850_otp_set_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_debug("%s, set OTP LSC to sensor OK.\n", __func__);
	return;
}
...
#endif
	pr_debug("%s, vcm_start:%#x, vcm_end:%#x", __func__, *vcm_start, *vcm_end);
}
#endif
...
static bool ov13850_get_otp_from_sensor(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	{
		pr_err("%s OTP data is worng, ov13850_otp_flag=0x%04x\n", __func__,otpflag);
		return false;
	}
	else if((otpflag & OV13850_OTP_CHECKSUM_READ) == OV13850_OTP_CHECKSUM_READ)
	{
		pr_debug("%s OTP has been read success already, ov13850_otp_flag=0x%01x\n", __func__,otpflag);
		return true;
	}
	...
	if (false == retVal)
	{
		pr_err("%s ov13850_otp_read_id() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal)
	{
		pr_err("%s ov13850_otp_read_awb() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (false == retVal)
	{
		pr_err("%s ov13850_otp_read_vcm() failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if (otpCheckSumVal == sum)
	{
		...
		otpflag = ov13850_otp_get_flag();
		pr_debug("%s success, OTPSUMVAL: %d, otpCheckSumVal: %d ,sum:%d, ov13850_otp_flag=0x%x\n", __func__, OTPSUMVAL, otpCheckSumVal,sum,otpflag);
		return true;
	}
	else
	{
...
OTP_FAIL:
	 ...
	 otpflag = ov13850_otp_get_flag();
	pr_debug("%s ov13850_otp_flag=0x%x \n", __func__,otpflag);
...
{
	int rc = 0;
	pr_debug("%s enters! i2c_addr = 0x%x.\n",__func__,i2c_addr);
...
int ov13850_sunny_p13v01h_otp_func(struct msm_sensor_ctrl_t * s_ctrl, int index)
{
	...
	pr_debug("%s, the OTP read and set end.\n", __func__);
	return rc;
}
