...
static char cabc_switch[2] = {0x53, 0x0};	/* DTYPE_DCS_WRITE1 */
static struct dsi_cmd_desc dimming_cmd = {
	{DTYPE_DCS_WRITE1, 1, 0, 0, 1, sizeof(cabc_switch)},
	cabc_switch
};

static void mdss_dsi_panel_dimming_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int on)
{
	struct dcs_cmd_req cmdreq;

	if(on==1)
	cabc_switch[1] = (unsigned char)0x2c;
	else
	cabc_switch[1] = (unsigned char)0x24;
	memset(&cmdreq, 0, sizeof(cmdreq));
...
static void hw_panel_bias_en(struct mdss_panel_data *pdata, int enable)
{
	...
	{
		if (ctrl_pdata->dimming_flag != 0) {/*dimming_flag:power on ==>0,  wakeup==>1, other==>2*/
			if(enable){
				ctrl_pdata->dimming_flag = 1;
			} else {
				ctrl_pdata->dimming_flag = 2;
			}
		}
	}
}
							...
							u32 bl_level)
{
	...
	if(ctrl_pdata->hw_product_pad)
	{
		if (ctrl_pdata->dimming_flag != 0) {
			if(1 == ctrl_pdata->dimming_flag) {
				ctrl_pdata->dimming_flag = 2;
				mdss_dsi_panel_dimming_dcs(ctrl_pdata,0);
			} else {
				mdss_dsi_panel_dimming_dcs(ctrl_pdata,1);
			}
		} else {
			ctrl_pdata->dimming_flag = 2;
			mdss_dsi_panel_dimming_dcs(ctrl_pdata,1);
...
#ifdef CONFIG_HUAWEI_LCD
	if(0 == strcmp(panel_name,"CMI_NT35521_8_800P_VIDEO") 
		|| 0 == strcmp(panel_name,"CMI_TOD_NT35521_8_800P_VIDEO")){
		pinfo->hw_lcd_type = 1;//cim
	}else{
		pinfo->hw_lcd_type = 0;//boe
	}
#endif
	if (!panel_name) {
