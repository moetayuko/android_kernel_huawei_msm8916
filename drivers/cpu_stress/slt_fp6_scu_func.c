...
__cpuc_flush_dcache_area((void*)g_iSCU_State, (size_t)2*sizeof(int));
...
static int wait_slt_scu_state_sync(unsigned long state, int wait)
{
    ...
    cpu_num  |= 0x1 << cpu;
    get_exit = 0;
    __cpuc_flush_dcache_area((void*)&get_exit, (size_t)sizeof(int));
    __cpuc_flush_dcache_area((void*)&cpu_num, (size_t)sizeof(int));
...
static int slt_scu_test_func(void *data)
{
    ...
    for (loop = 0; loop < g_iScuLoopCount; loop++) {
        ...
        g_iSCU_PassFail[cpu] = fp6_scu_start((unsigned long)mem_buf);
        spin_unlock_irqrestore(&scu_thread_irq_lock[cpu], irq_flag);
