...
#ifdef CONFIG_HUAWEI_KERNEL 
    /* 
    When free size is less then 30%, not hide reserved space
    resv_blocks is not care here

    F = f_blocks,  total blocks
    R = ext4_r_blocks_count(es), Reserved blocks
    
    f_bavail = f_bfree - R;      when  f_bfree < 30%*F  
    f_bavail = f_bfree - (1-(f_bfree-0.3*F)/(0.7*F))R;      when  f_bfree >= 30%*F  
    */
    if(NULL != sb->s_id){
        if(!strcmp(sb->s_id, USERDATA)){
            /* only process data partition */
            bflex_point = (long)buf->f_blocks * 3 / 10;
            if (buf->f_bfree < (ext4_r_blocks_count(es) + resv_blocks)){
                buf->f_bavail = 0;
            }else if(buf->f_bfree < bflex_point){
                buf->f_bavail = buf->f_bfree -  (ext4_r_blocks_count(es) + resv_blocks);
            }else{
                buf->f_bavail = buf->f_bfree - 
                     ((100 - ((long)buf->f_bfree - bflex_point) * 100 / ((long)buf->f_blocks - bflex_point)) * ((long)ext4_r_blocks_count(es)) / 100 + resv_blocks);  
            }
